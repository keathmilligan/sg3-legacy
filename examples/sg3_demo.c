...
#elif defined(__APPLE__)
#define RESOURCE_DIR resources_directory
char resources_directory[512];
void getResourceDirectory() {
    CFBundleRef mainBundle = CFBundleGetMainBundle();
    CFURLRef resourceURL = CFBundleCopyResourcesDirectoryURL(mainBundle);
    CFURLGetFileSystemRepresentation(resourceURL, TRUE, (UInt8*)resources_directory, sizeof(resources_directory));
    strcat(resources_directory, "/");
    LOG("resource directory: %s\n", resources_directory);
}
#else
#define RESOURCE_DIR "resources/"
#endif
